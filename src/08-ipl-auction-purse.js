...
export function iplAuctionSummary(team, players) {
  if (typeof team !== "object" || Array.isArray(team) || !team || typeof team.purse !== "number" || team.purse <= 0 || !Array.isArray(players) || players.length === 0) {
    return null
  }

  const totalSpent = players.reduce((total , player) => total+player.price, 0)
  ...
  const costliestPlayer = sortedPlayers[0]
  const cheapestPlayer = sortedPlayers[sortedPlayers.length - 1]
